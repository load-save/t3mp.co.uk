...
document.addEventListener("DOMContentLoaded", function() {
  const elem = document.querySelector("#animation-container");
  if (elem) {
  ...
  function parallax(e) {
    ...
    let _depth1 = `${50 + (_mouseX - _w) * 0.0075}% ${50 + (_mouseY - _h) * 0.0075 * 2.25}%`;
    let _depth2 = `${50 + (_mouseX - _w) * 0.015}% ${50 + (_mouseY - _h) * 0.015* 2.25}%`;
    let _depth3 = `${50 + (_mouseX - _w) * 0.02}% ${50 + (_mouseY - _h) * 0.02* 2.25}%`;  
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  showContentButton.addEventListener("click", function () {
      ...
      // if on mobile, keep the button hidden
...
nextButton.addEventListener("mouseenter", function () {
  nextimg.src = nextimg.getAttribute("data-hover-src");
  hover.currentTime = 0;
  hover.volume = .175;
hover.play();
});
...
nextButton.addEventListener("mouseleave", function () {
  nextimg.src = nextimg.getAttribute("data-original-src");
});

prevButton.addEventListener("mouseenter", function () {
  previmg.src = previmg.getAttribute("data-hover-src");
  hover.currentTime = 0;
  hover.volume = .175;
hover.play();
});
...
prevButton.addEventListener("mouseleave", function () {
  previmg.src = previmg.getAttribute("data-original-src");
});
...
const playButton = document.getElementById('showContentButton');
